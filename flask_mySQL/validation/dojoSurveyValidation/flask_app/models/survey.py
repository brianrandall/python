...
class Survey:
    ...
    def create(cls,data):
        ...
        return connectToMySQL('dojoSurveyValidation').query_db(query,data)

    @classmethod
    def get_last_survey(cls):
        query = "SELECT * FROM surveys ORDER BY surveys.id DESC LIMIT 1;"
        results = connectToMySQL('dojoSurveyValidation').query_db(query)
    ...
    def is_valid(survey):
        ...
        if len(survey['location']) < 1:
            is_valid = False
            flash("must enter location")
        if len(survey['language']) < 1:
            is_valid = False
            flash("choose language, dummy")
        if len(survey['comment']) < 3:
            is_valid = False
            flash("enter a real comment")
        return is_valid